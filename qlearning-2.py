...
def q_learning():
    ...
    optimize = 0 #aynı adım sayısı alınırsa artar
    adimlar=[]
    oranlar=[]
    basarisiz = True
    onceki_as = 0
    enbasarili = -500
    while basarisiz and optimize < 30:
            rota, step_n, odul, basarisiz = episode()
            oran = odul/step_n
            if oran > enbasarili:
                enbasarili = oran
                optimize = 0
            elif oran == enbasarili:
                optimize += 1
            label['text'] = "En başarılı %f, optimize %i" % (enbasarili,optimize)
            pencere.update_idletasks()
#            if step_n == onceki_as:
#                optimize += 1
#            else:
...
def episode():
    ...
    while durum != hedef_durum:
        eylem, durum, yeniodul=step(durum)
        odul += yeniodul
        step_n += 1
        rota.append(eylem)
        if yeniodul == engel_odulu:
            carpti = True
            
    for d in G:
        for e in range(0,8):
            d[e] = 0 #geçildi değerlerini sıfırla
    return rota, step_n, odul, carpti
...
def rota_ciz(rota):
    ...
    for eylem in rota:
        ...
        odul += R[durum][eylem]
        durum = yeni_durum
    #üzerine yazdırılmış olabileceği için başlangıç ve hedef yeniden çizilir
